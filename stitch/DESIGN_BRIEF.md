...
- FieldLens Audit Q9M7 turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser field inspection app called FieldLens Audit Q9M7. It should manage inspection routes, stop checklists, issue capture, evidence notes, offline/empty/error states, and every visible action should update real local app state.
...
- Inspection Operations - FieldLens Audit Q9M7
- Inspection Editor - FieldLens Audit Q9M7
- Empty and Error Recovery - FieldLens Audit Q9M7

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Inspection Operations - FieldLens Audit Q9M7
- surface_id: SURF_INSPECTION_OPERATIONS
- unique_canvas_caption: Inspection Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Inspection data.
...
- data_entities: Inspection, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Inspection Editor - FieldLens Audit Q9M7
- surface_id: SURF_INSPECTION_EDITOR
- unique_canvas_caption: Inspection Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Inspection changes.
...
- data_entities: Inspection, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_INSPECTION_OPERATIONS -> Save returns to SURF_INSPECTION_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Empty and Error Recovery - FieldLens Audit Q9M7
...
- purpose: Keep the Inspection workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_INSPECTION_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Inspection workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_INSPECTION_OPERATIONS - Inspection Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Inspection data.
   Data: Inspection, ActivityEvent, Preference
...
2. SURF_INSPECTION_EDITOR - Inspection Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Inspection changes.
   Data: Inspection, ValidationError
   ...
   Entry/exit: SURF_INSPECTION_OPERATIONS -> Save returns to SURF_INSPECTION_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
   Guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

3. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   Purpose: Keep the Inspection workflow usable when data is missing, filtered away, loading, failed, or corrupt.
   Data: Inspection, ActivityEvent, Preference
   ...
   Entry/exit: SURF_INSPECTION_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Inspection workflow and preserves unrelated state.
